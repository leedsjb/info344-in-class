...
// ResponseWriter is an interface (so no *), interfaces are always passed by reference
// Request is a struct
// (zi zipIndex) is the receiver, same as "this" in Java
...
func main() {
	...
	// to a zipSlice ( []*zip )
	for _, z := range zips { // iterate over pointers to zipcodes; _ allows you to ignore var created and not used
		lower := strings.ToLower(z.City) // account for lowercase / uppercase
		zi[lower] = append(zi[lower], z) // for each iteration of the loop append z to zi(lower)
	}
	...
	// zi before zipsForCityHandler to account for receiver
	// same as calling a function on an object
